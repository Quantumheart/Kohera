...
IncomingShare _share({String? text, List<IncomingShareFile> files = const []}) =>
    IncomingShare(text: text, files: files);
...
void main() {
  ...
  test('file share reads bytes and sends file event', () async {
    ...
    await sendIncomingShareToRoom(
      ...
      '!room:server',
      _share(files: [
        IncomingShareFile(filePath: file.path, name: 'pic.png', mimeType: 'image/png'),
      ]),
    );
  ...
  test('missing staged file is skipped, no throw', () async {
    await sendIncomingShareToRoom(
      client,
      '!room:server',
      _share(files: [const IncomingShareFile(filePath: '/no/such/file', name: 'x')]),
    );
    verifyNever(room.sendFileEvent(any,
        threadRootEventId: anyNamed('threadRootEventId'),
        threadLastEventId: anyNamed('threadLastEventId')));
