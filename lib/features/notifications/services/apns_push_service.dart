...
class ApnsPushService {
  ...
  Future<void> init() async {
    ...
    debugPrint('[Kohera] ApnsPushService initialized');
  }
  ...
  Future<dynamic> _handleMethodCall(MethodCall call) async {
    switch (call.method) {
      case 'onToken':
        ...
        debugPrint(
          '[Kohera] APNs token received: ${token.substring(0, token.length.clamp(0, 8))}...',
        );
        _currentToken = token;
      ...
      case 'onNotificationReply':
        ...
        debugPrint('[Kohera] APNs inline reply for room $roomId');
        unawaited(_handleInlineReply(roomId, text));
      case 'onNotificationMarkAsRead':
        final args = Map<String, dynamic>.from(call.arguments as Map);
        final roomId = args['roomId'] as String;
        final eventId = args['eventId'] as String?;
        debugPrint('[Kohera] APNs mark as read for room $roomId');
  ...
  Future<void> register() async {
    ...
    try {
      ...
      debugPrint('[Kohera] APNs registration requested');
    } on PlatformException catch (e) {
      debugPrint('[Kohera] APNs registration failed: ${e.message}');
    }
  }
  ...
  Future<void> unregister() async {
    ...
    try {
      ...
      debugPrint('[Kohera] Failed to unregister APNs pusher: $e');
    }
    try {
      await _channel.invokeMethod<void>('unregister');
    } catch (e) {
      debugPrint('[Kohera] Failed to unregister from APNs: $e');
    }
    _currentToken = null;
    debugPrint('[Kohera] APNs unregistered');
  }
  ...
  Future<void> _registerPusher(String token) async {
    ...
    try {
      ...
      debugPrint('[Kohera] APNs pusher registered with gateway $gatewayUrl');
    } catch (e) {
      debugPrint('[Kohera] Failed to register APNs pusher: $e');
    }
  }
  ...
  Future<void> _handleInlineReply(String roomId, String text) async {
    ...
    try {
      ...
      debugPrint('[Kohera] Inline reply sent to room $roomId');
    } catch (e) {
      debugPrint('[Kohera] Failed to send inline reply: $e');
    }
  }
  ...
  Future<void> _handleMarkAsRead(String roomId, String? eventId) async {
    ...
    try {
      ...
      debugPrint('[Kohera] Marked room $roomId as read');
    } catch (e) {
  ...
  static Future<void> clearBadge() async {
    ...
    try {
      ...
      debugPrint('[Kohera] Failed to clear badge: $e');
    }
  }
