...
class MediaCache {
  ...
  static Future<ResolvedMedia> resolve(MediaController controller) async {
    final cached = _tempFiles[controller.eventId];
    if (cached != null && !kIsWeb && File(cached).existsSync()) {
      _promote(controller.eventId);
      return ResolvedMedia(filePath: cached, mimeType: controller.mimeType);
    }
  ...
  static Future<ResolvedMedia> _bytesToMedia(
      String eventId, Uint8List bytes, String? mimetype,) async {
    if (kIsWeb) {
      return ResolvedMedia(bytes: bytes, mimeType: mimetype);
    }
    ...
    _evictOldest();
    return ResolvedMedia(filePath: path, mimeType: mimetype);
  }
