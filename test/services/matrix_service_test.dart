...
void main() {
  ...
  group('spaces getter', () {
    test('filters rooms to only spaces and sorts alphabetically', () {
      ...
      final spaces = service.spaceTree.spaces;
  ...
  group('rooms getter', () {
    test('filters out spaces and sorts by last event timestamp', () {
      ...
      final rooms = service.spaceTree.rooms;
  ...
  group('invitedRooms', () {
    test('returns only non-space rooms with invite membership, sorted alphabetically', () {
      ...
      final result = service.spaceTree.invitedRooms;
    ...
    test('returns empty list when no invites', () {
      ...
      expect(service.spaceTree.invitedRooms, isEmpty);
    });
  });
  ...
  group('invitedSpaces', () {
    test('returns only spaces with invite membership, sorted alphabetically', () {
      ...
      final result = service.spaceTree.invitedSpaces;
      expect(result, hasLength(2));
  ...
  group('inviterDisplayName', () {
    test('returns display name of the inviter', () {
      ...
      final result = service.spaceTree.inviterDisplayName(room);
      expect(result, 'Alice');
    });
    ...
    test('returns null when no userID', () {
      ...
      expect(service.spaceTree.inviterDisplayName(room), isNull);
    });
  ...
  group('space tree', () {
    ...
    test('builds correctly with nested subspaces', () {
      final tree = service.spaceTree.spaceTree;
    ...
    test('orphanRooms excludes rooms in any space', () {
      final orphans = service.spaceTree.orphanRooms;
    ...
    test('roomsForSpace returns correct children', () {
      final roomsA = service.spaceTree.roomsForSpace('!spaceA:example.com');
      expect(roomsA, hasLength(1));
      expect(roomsA[0].id, '!room1:example.com');

      final roomsSub = service.spaceTree.roomsForSpace('!subspace:example.com');
      expect(roomsSub, hasLength(1));
      expect(roomsSub[0].id, '!room2:example.com');

      final roomsB = service.spaceTree.roomsForSpace('!spaceB:example.com');
    ...
    test('roomsForSpace sorts by order field then alphabetically', () {
      ...
      when(mockClient.getRoomById('!room3:example.com')).thenReturn(room3);
      service.spaceTree.invalidateSpaceTree();

      final rooms = service.spaceTree.roomsForSpace('!spaceA:example.com');
    ...
    test('spaceMemberships returns correct set', () {
      final memberships1 = service.spaceTree.spaceMemberships('!room1:example.com');
      expect(memberships1, {'!spaceA:example.com'});

      final memberships2 = service.spaceTree.spaceMemberships('!room2:example.com');
      expect(memberships2, {'!subspace:example.com'});

      // Orphan room has no memberships
      final orphanMemberships = service.spaceTree.spaceMemberships('!orphan:example.com');
      expect(orphanMemberships, isEmpty);
    });

    test('unreadCountForSpace aggregates correctly', () {
      // Space A: room1 (3) + subspace room2 (5) = 8
      expect(service.spaceTree.unreadCountForSpace('!spaceA:example.com'), 8);

      // Subspace alone: room2 (5) = 5
      expect(service.spaceTree.unreadCountForSpace('!subspace:example.com'), 5);

      // Space B: room3 (0) = 0
      expect(service.spaceTree.unreadCountForSpace('!spaceB:example.com'), 0);

      // Non-existent space = 0
      expect(service.spaceTree.unreadCountForSpace('!nonexistent:example.com'), 0);
    });

    test('rooms getter returns all non-space rooms unfiltered', () {
      service.selectionController.selectSpace('!spaceA:example.com');

      final allRooms = service.spaceTree.rooms;
    ...
    test('updateSpaceOrder changes ordering of spaces getter', () {
      ...
      expect(service.spaceTree.spaces[0].id, '!spaceA:example.com');
      expect(service.spaceTree.spaces[1].id, '!spaceB:example.com');

      // Custom: B before A
      service.spaceTree.updateSpaceOrder([
        '!spaceB:example.com',
        '!spaceA:example.com',
      ]);

      expect(service.spaceTree.spaces[0].id, '!spaceB:example.com');
      expect(service.spaceTree.spaces[1].id, '!spaceA:example.com');
    });
    ...
    test('new spaces not in custom order appear at the end alphabetically',
      ...
      service.spaceTree.updateSpaceOrder(['!spaceB:example.com']);
      ...
      // Force tree rebuild
      service.spaceTree.updateSpaceOrder(['!spaceB:example.com', '!spaceA:example.com']);
