...
void main() {
  ...
  group('IncomingShare', () {
    test('text-only round-trip', () {
      const share = IncomingShare(roomId: '!r:s', text: 'hello world');
      final decoded = IncomingShare.decode(share.encode());
      expect(decoded.roomId, '!r:s');
    ...
    test('isEmpty when blank', () {
      const blank = IncomingShare(roomId: '!r:s', text: '');
      expect(blank.isEmpty, isTrue);
    });

    test('isEmpty when null text and no files', () {
      const blank = IncomingShare(roomId: '!r:s');
      expect(blank.isEmpty, isTrue);
    });
